...
public partial class Sun : Drops
{
	...
	public override void _Drop()
	{
		...
		Tween tweenScale = CreateTween();
		tweenScale.TweenProperty(this, "scale", new Vector2(1, 1), 0.25f).SetEase(Tween.EaseType.InOut).SetTrans(Tween.TransitionType.Linear);

		tween1.TweenProperty(this, "position", new Vector2(Position.X + rng_x, Position.Y - rng_y), 0.2f).SetEase(Tween.EaseType.OutIn).SetTrans(Tween.TransitionType.Linear);

		//await ToSignal(GetTree().CreateTimer(0.1f), "timeout");
		//Tween tween2 = CreateTween();
		tween1.TweenProperty(this, "position", new Vector2(Position.X + rng_x * 1.5f, Position.Y + 45), 0.3f).SetEase(Tween.EaseType.In).SetTrans(Tween.TransitionType.Linear);
	}
}
